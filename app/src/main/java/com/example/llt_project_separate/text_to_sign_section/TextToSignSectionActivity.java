...
//        textSignPairs.add(new TextSignPair(" ", R.drawable.space));
       ...
       UtilsRetrofit.getInstance(this).getAllTextSignPairs(new Callback<List<TextSignPair>>() {
          @Override
          public void onResponse(@NonNull Call<List<TextSignPair>> call, @NonNull Response<List<TextSignPair>> response) {
             if (response.isSuccessful()) {
                List<TextSignPair> textSignPairs = response.body();
                for (TextSignPair textSignPair : textSignPairs) {
                   String letter = textSignPair.getLetter();
                   String name = letter == " " ? "space" : "dactilema_" + letter.replace(" ", "_").toLowerCase();
                   int drawableId = getResources().getIdentifier(name, "drawable", getPackageName());
                   textSignPair.setImageSource(drawableId);
                }
                textToSignSectionRecyclerViewAdapter.setTextSignPairs(textSignPairs);
                ...
                Toast.makeText(TextToSignSectionActivity.this, "Obtinerea subcategoriilor perechi litera-simbol a esuat!", Toast.LENGTH_SHORT).show();
             }
          }

          @Override
          public void onFailure(@NonNull Call<List<TextSignPair>> call, @NonNull Throwable throwable) {
             Toast.makeText(TextToSignSectionActivity.this, "Ceva nu a mers bine! Încearcă din nou!", Toast.LENGTH_SHORT).show();
          }
       });
    }
